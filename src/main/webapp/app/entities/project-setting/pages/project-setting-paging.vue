...
import ProjectSettingForm from '../form/project-setting-form.vue';
import ProjectSettingDelete from '../delete/project-setting-delete.vue';
import ProjectSettingView from '../view/project-setting-view.vue';
import { useProjectSettingPaging } from './project-setting-paging';

const { state, meta, actions } = useProjectSettingPaging();

function maskedValue(row: any): string {
  if (row.valueType === 'PASSWORD' && row.settingValue) {
    return '••••••••';
  }
  ...
  <div class="q-pa-md">
    ...
    >
      ...
      <template #top>
        <div class="row items-center full-width">
          <div class="text-h6">{{ $t('projectSetting.title') }}</div>
          <q-space />
      ...
      <!-- VALUE (masked for PASSWORD) -->
      <template #body-cell-settingValue="props">
        <q-td :props="props">
          <span>{{ maskedValue(props.row) }}</span>
        </q-td>
      </template>

      <!-- VALUE TYPE BADGE -->
      <template #body-cell-valueType="props">
        <q-td :props="props">
          <q-badge
            :color="
      ...
      <q-card style="min-width: 550px">
        <q-toolbar class="bg-primary text-white">
          <q-toolbar-title>
            ...
            {{ state.formDialog.mode === 'edit' ? $t('projectSetting.editTitle') : $t('projectSetting.createTitle') }}
          </q-toolbar-title>
    ...
    <q-dialog v-model="state.deleteDialog.show" persistent>
      <q-card style="min-width: 450px">
        ...
        <ProjectSettingDelete :item="state.deleteDialog.item" @deleted="actions.handleDeleted" @cancel="actions.closeDeleteDialog()" />
      </q-card>
    </q-dialog>
